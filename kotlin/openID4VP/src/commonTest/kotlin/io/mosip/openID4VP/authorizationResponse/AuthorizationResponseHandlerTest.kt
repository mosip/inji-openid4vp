        ...
        verify(exactly = 1) {
            mockResponseHandler.sendAuthorizationResponse(
                ...
                authorizationResponse = match {
                    ...
                    assertEquals("VPTokenElement(value=MdocVPToken(base64EncodedDeviceResponse=base64EncodedDeviceResponse))",
                        it.vpToken.toString())
                    assertEquals("PresentationSubmission(id=649d581c-f291-4969-9cd5-2c27385a348f, definitionId=649d581c-f891-4969-9cd5-2c27385a348f, descriptorMap=[DescriptorMap(id=input2, format=mdoc_vp, path=$, pathNested=null)])",
    ...
    fun `should share credentials for 2LDP, 2SD-JWT and 2MSO-MDOC VC`() {
        ...
        every {
            ...
            listOf(ldpVPToken2), listOf(
                DescriptorMap("input1", "ldp_vp", "$[2]", PathNested("input1", "ldp_vc", "$.verifiableCredential[0]")),
                DescriptorMap("input1", "ldp_vp", "$[2]", PathNested("input1", "ldp_vc", "$.verifiableCredential[1]"))
            ), 2
        )
        every {
            anyConstructed<MdocVPTokenBuilder>().build(
